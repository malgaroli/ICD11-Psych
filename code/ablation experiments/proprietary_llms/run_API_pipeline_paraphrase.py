...
def main():
    ...
    results_folder_tmp = "results_paraphrased"
    ...
    for llm in llm_model: 
        ...
        for level in levels:
            ...
            results_folder = base_path.joinpath(results_folder_tmp,llm,level)
            results_folder.mkdir(parents=True, exist_ok=True)
            # load vignettes with labels
            data_path = base_path.joinpath("data","vignettes_paraphrased",f"Data_final_updated_v1_{level}.csv")
            ...
            for vp in all_prompts:
                ...
                df_backup.to_csv(backup_folder.joinpath(f"zeroShot_{llm}_{id}_{level}.csv"), index=False)
