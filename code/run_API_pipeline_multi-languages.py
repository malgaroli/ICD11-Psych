...
def extract_ranked_diagnoses(response, language):
    ...
    if language == "english":
        ...
        alt_pattern1 = r"(?:1\.|First|Most Likely).*?Diagnosis:\s*(.+?)(?=\s*Reasoning:|\n|$)"
        alt_pattern2 = r"(?:2\.|Second|Second Most Likely).*?:\s*(.+?)(?=\s*Reasoning:|\n|$)"
        alt_pattern3 = r"(?:3\.|Third|Third Most Likely).*?:\s*(.+?)(?=\s*Reasoning:|\n|$)"
        ...
        alt_pattern1 = r"(?:Diagnostic le plus probable|1\.\s*Diagnostic)\s*:\s*(.+?)(?=\s*Justification\s*:|\n|$)"
        alt_pattern2 = r"(?:Deuxième diagnostic le plus probable|2\.)\s*:\s*(.+?)(?=\s*Justification\s*:|\n|$)"
        alt_pattern3 = r"(?:Troisième diagnostic le plus probable|3\.)\s*:\s*(.+?)(?=\s*Justification\s*:|\n|$)"
        ...
        alt_pattern1 = r"(?:Diagnóstico más probable|1\.)\s*:\s*(.+?)(?=\s*Justificación\s*:|\n|$)"
        alt_pattern2 = r"(?:Segundo diagnóstico más probable|2\.)\s*:\s*(.+?)(?=\s*Justificación\s*:|\n|$)"
        alt_pattern3 = r"(?:Tercer diagnóstico más probable|3\.)\s*:\s*(.+?)(?=\s*Justificación\s*:|\n|$)"
        ...
        alt_pattern1 = r"(?:Наиболее вероятный диагноз|1\.)\s*:\s*(.+?)(?=\s*Обоснование\s*:|\n|$)"
        alt_pattern2 = r"(?:Второй по вероятности диагноз|2\.)\s*:\s*(.+?)(?=\s*Обоснование\s*:|\n|$)"
        alt_pattern3 = r"(?:Третий по вероятности диагноз|3\.)\s*:\s*(.+?)(?=\s*Обоснование\s*:|\n|$)"

    elif language == "chinese":
        pattern1 = r"(?:\*\*)?\s*(?:最可能的诊断|最有可能的诊断|1\.)\s*[:：]\s*(?:\*\*)?\s*(.+?)(?=\s*理由\s*[:：]|\n|$)"
        pattern2 = r"(?:\*\*)?\s*(?:第二可能的诊断|第二最可能的诊断|2\.)\s*[:：]\s*(?:\*\*)?\s*(.+?)(?=\s*理由\s*[:：]|\n|$)"
        pattern3 = r"(?:\*\*)?\s*(?:第三可能的诊断|第三最可能的诊断|3\.)\s*[:：]\s*(?:\*\*)?\s*(.+?)(?=\s*理由\s*[:：]|\n|$)"

        alt_pattern1 = r"(?:最可能的诊断|1\.)\s*[：:]\s*(.+?)(?=\s*理由\s*[：:]|\n|$)"
        alt_pattern2 = r"(?:第二可能的诊断|2\.)\s*[：:]\s*(.+?)(?=\s*理由\s*[：:]|\n|$)"
        alt_pattern3 = r"(?:第三可能的诊断|3\.)\s*[：:]\s*(.+?)(?=\s*理由\s*[：:]|\n|$)"

    elif language == "japanese":
        pattern1 = r"(?:\*\*)?\s*(?:最も可能性の高い診断|1\.)\s*[:：]\s*(?:\*\*)?\s*(.+?)(?=\s*理由\s*[:：]|\n|$)"
        pattern2 = r"(?:\*\*)?\s*(?:次に可能性が高い診断|次に可能性の高い診断|2\.)\s*[:：]\s*(?:\*\*)?\s*(.+?)(?=\s*理由\s*[:：]|\n|$)"
        pattern3 = r"(?:\*\*)?\s*(?:3番目に可能性が高い診断|3番目に可能性の高い診断|3\.)\s*[:：]\s*(?:\*\*)?\s*(.+?)(?=\s*理由\s*[:：]|\n|$)"

        alt_pattern1 = r"(?:最も可能性の高い診断|1\.)\s*[：:]\s*(.+?)(?=\s*理由\s*[：:]|\n|$)"
        alt_pattern2 = r"(?:次に可能性が高い診断|2\.)\s*[：:]\s*(.+?)(?=\s*理由\s*[：:]|\n|$)"
        alt_pattern3 = r"(?:3番目に可能性が高い診断|3\.)\s*[：:]\s*(.+?)(?=\s*理由\s*[：:]|\n|$)"
...
def main():
    ...
    results_folder_tmp = "results_languages"
    ...
    for llm in llm_model: 
        ...
        for language_vignette in languages_vignette:
            ...
            if language_vignette == "english":
                ...
                data_path = base_path.joinpath("data","multi-languages-corrected",f"{language_vignette}",f"Data_final_{language_vignette}.csv")
            ...
            prompt_builder = PromptBuilder(df_vignettes=data, prompts_path=prompt_path, prompt_id=prompt_id, language=language_vignette)
            ...
            evaluate_model_outputs(data, all_prompts, list_responses, index_mapping, llm, results_folder, prompt_id, language_vignette)
